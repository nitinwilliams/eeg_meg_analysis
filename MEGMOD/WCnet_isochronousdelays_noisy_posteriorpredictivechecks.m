function WCnet_isochronousdelays_noisy_posteriorpredictivechecks(array_idx)
%% Initialisation
...
global PY2LV_transvec WCnet_noisy_isochronousdelays_posteriordistribution avg_sc source2parc fwd_mat inv_mat
toc;
...
num_reps=5;
mod_params=WCnet_noisy_isochronousdelays_posteriordistribution(((array_idx-1)*num_reps)+1:(array_idx*num_reps),:);
...
for rep_idx=1:num_reps
        ...
        tic;
        delays_sd=(mod_params(rep_idx,13)*mod_params(rep_idx,12));
        delays_matrix(:,rep_idx)=ceil(abs((delays_sd*randn(length(ut_idxs),1))+mod_params(rep_idx,12)));

        Tij=zeros(N);
        Tij(ut_idxs)=delays_matrix(:,rep_idx);
        Tij=Tij+Tij';
        Tij(1:N+1:end)=0;
      ...
      save(['/scratch/work/willian1/MEGMOD/MATLAB/data/WCnet_isochronousdelays_noisy_60s_posteriorpredictivechecks_batch',int2str(array_idx),'.mat'],...
      'wPLImags_acrossreps','PLVmags_acrossreps','R_acrossreps','delays_matrix','mod_params');
