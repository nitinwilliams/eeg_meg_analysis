function out = simulate_WCnetnoisy_isochronousdelays(wee,wei,wie,wii,be,bi,taue,taui,k,IH,psi_sigma,delay,delay_cv)
...
delay_cv_scaled=0.05*delay_cv+0.2;

% Parameters for Wilson-Cowan network model

delay_sd_scaled=(delay_cv_scaled*delay_scaled);
delays_vector=ceil(abs((delay_sd_scaled*randn(sum(ut_idxs),1))+delay_scaled));

Tij=zeros(N);
Tij(ut_idxs)=delays_vector;
Tij=Tij+Tij';
Tij(1:N+1:end)=0;
