...
4. line 20 indicates names of files which point to input datasets.
   ...
   input data (as specified in line 29). change the filenames/variables
...
5. lines 31-33 are specific to our BCI dataset and can be commented out.
   note that to run line 35 and onwards, the variable bcidata_3D needs 
   to contain input data in the format: 
   channels x samples x trials of both conditions

6. lines 37 to 50 perform baseline correction. 'bpts' specifies number of samples
...
7. lines 54 to 60 is code for z-scoring each trial

8. lines 64 to 83 is code for z-scoring across trials

9. lines 107 to 143 segments dataset into a set of 'windows', which will also be the 
...
10. lines 147-165 estimate sparsity level parameter for 10% of randomly selected 
    segments, for model orders from 1 to 6. this percentage is specified in pctval.

11. lines 169-196 determine optimal model order according to BCI, by estimating mode
    of model order estimated from 10% of randomly selected segments (given by pctval_2).

12. lines 200-216 estimate the sparse-MVAR model for each segment, with the selected
...
14. from line 230 onwards, variable CMAT is subject to clustering via k-means algorithm
    where k is varied according to the range previously specified.

15. from lines 240-249, the output of the clustering stage is re-ordered into the
...
16. lines 259-271 perform Hidden Markov Modelling on 'symbols' from both conditions
    separately, after initialising the emission matrix to identity in both cases.

17. from lines 275-385, n-fold cross-validation is performed to determine if 
    trials from two conditions can be classified with above-chance accuracy.
    number of folds is set to 2 by parameter 'numfolds'. null distribution of 
    accuracy is determined via a permutation-based procedure. classification 
    accuracy for each subject, for each value of 'k' is in variable CA

18. from lines 389-448, another performance measure, i.e. model distance is also
    assessed for statistical significance, to determine if HMMs estimated from both
...
19. lines 452-511 generate p-values of CA and MD for each subject for each value of 'k'
    p-values for CA are in 'pval_permCA' and p-values for MD are in 'pval_MD'

20. Good luck! please e-mail nitinwilliams@gmail.com if you have any questions!
