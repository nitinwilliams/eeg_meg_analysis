...
%% Obtain hem-wise matrices

fname=strcat('SC_parc2k9_57HCP.mat');
load(fname,'sc_all');

reps=100;
num_subs=size(sc_all,1);

SCmats_groupmat=zeros(N*2,N*2,reps);

for repidx=1:reps,
    
    sub_idxs=randi(num_subs,[num_subs,1]);
    D=squeeze(mean(sc_all(sub_idxs,:,:),1));
    D=D+D';
    SCmats_groupmat(:,:,repidx)=D;
    
end
       
save(['B:\Nitin\SCFC_methods\data\sc_data\SC_surrmats_parc2k9_57HCP.mat'],'SCmats_groupmat');
