...
load(['B:\Nitin\SCFC_methods\data\meg_data\Sb_weighted_FC_MEG_surr_gamma' num2str(gamma) '_pctval' int2str(pctval1) '_thr' int2str(nct) '_lefthem_wTFC_parc2k9.mat'],'Sb');
Sb_surr_MEG=Sb;

load(['B:\Nitin\SCFC_methods\data\meg_data\Pstabmat_weighted_Louvain_FC_MEG_lefthem_parc2k9_67subs_gamma' num2str(gamma) '_pct' int2str(pctval1) '.mat'],'Pstabmat','Pstabmat_rand');  
Pstabmat_thr=squeeze(prctile(Pstabmat_rand,95,2));
MASK_MEG=Pstabmat>Pstabmat_thr;

load(['B:\Nitin\SCFC_methods\data\sc_data\Sb_weighted_SC_surr_gamma' num2str(gamma) '_pctval' int2str(pctval2) '_thr' int2str(nct) '_lefthem_wTFC_parc2k9.mat'],'Sb');
Sb_surr_SC=Sb;

load(['B:\Nitin\SCFC_methods\data\sc_data\Pstabmat_weighted_Louvain_SC_lefthem_parc2k9_67subs_gamma' num2str(gamma) '_pct' int2str(pctval2) '.mat'],'Pstabmat','Pstabmat_rand');  
Pstabmat_thr=squeeze(prctile(Pstabmat_rand,95,2));
MASK_SC=Pstabmat>Pstabmat_thr;

MASK=(MASK_MEG.*MASK_SC);

numreps=size(Sb_surr_MEG,1);
PS_lefthem=zeros(1,numreps);
PS_nulldist_lefthem=zeros(perms,numreps);

for ridx=1:numreps,
     
   Sb_MEG=Sb_surr_MEG{ridx,1};
...
load(['B:\Nitin\SCFC_methods\data\meg_data\Sb_weighted_FC_MEG_surr_gamma' num2str(gamma) '_pctval' int2str(pctval1) '_thr' int2str(nct) '_righthem_wTFC_parc2k9.mat'],'Sb');
Sb_surr_MEG=Sb;

load(['B:\Nitin\SCFC_methods\data\meg_data\Pstabmat_weighted_Louvain_FC_MEG_righthem_parc2k9_67subs_gamma' num2str(gamma) '_pct' int2str(pctval1) '.mat'],'Pstabmat','Pstabmat_rand');  
Pstabmat_thr=squeeze(prctile(Pstabmat_rand,95,2));
MASK_MEG=Pstabmat>Pstabmat_thr;

load(['B:\Nitin\SCFC_methods\data\sc_data\Sb_weighted_SC_surr_gamma' num2str(gamma) '_pctval' int2str(pctval2) '_thr' int2str(nct) '_righthem_wTFC_parc2k9.mat'],'Sb');
Sb_surr_SC=Sb;

load(['B:\Nitin\SCFC_methods\data\sc_data\Pstabmat_weighted_Louvain_SC_righthem_parc2k9_67subs_gamma' num2str(gamma) '_pct' int2str(pctval2) '.mat'],'Pstabmat','Pstabmat_rand');  
Pstabmat_thr=squeeze(prctile(Pstabmat_rand,95,2));
MASK_SC=Pstabmat>Pstabmat_thr;
...
save(['B:\Nitin\SCFC_methods\data\meg_data\partsim_weighted_Louvain_SC_MEG_FC_' Nf{1,1} 'Hz_parc2k9_67subs_gamma' num2str(gamma) '.mat'],'PS_lefthem','PS_righthem','PS_bothhems');
save(['B:\Nitin\SCFC_methods\data\meg_data\zpartsim_weighted_Louvain_SC_MEG_FC_' Nf{1,1} 'Hz_parc2k9_67subs_gamma' num2str(gamma) '.mat'],'zPS_lefthem','zPS_righthem','zPS_bothhems');
